...
1. Clone the repository
   ...
   cd wtist-stablecoin
   ```
2. Set up environment variables - Create a `.env` file within the folder `/backend`.

   ```shell
   PROVIDER_URL=https://sepolia-rollup.arbitrum.io/rpc # Replace with your Arbitrum Sepolia RPC provider URL
   PRIVATE_KEY=<private-key-of-wallet> # Address of the wallet that will deploy the smart contracts
   ```
