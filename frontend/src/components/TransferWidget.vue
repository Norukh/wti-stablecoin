...
async function approveTransfer() {
  ...
  if (transferInfo.value.amount && balance < transferInfo.value.amount) {
    ...
    toast.add({ severity: 'error', summary: 'Insufficient balance', detail: 'Your balance is not enough to complete the transfer', life: 5000 })
    return
  }
  ...
  try {
    ...
    const tx = await contract.approve(signerAddress, transferInfo.value.amount)
    ...
    toast.add({ severity: 'error', summary: 'Error approving WTIST transfer', detail: error.message.toString(), life: 5000 })
  ...
  try {
    ...
    const tx = await contract.transferFrom(signerAddress, transferInfo.value.recipientAddress, transferInfo.value.amount)
    console.log("Transaction hash: ", tx.hash)
    ...
    toast.add({ severity: 'success', summary: `WTIST transferred successfully`, detail: `Gas used: ${receipt.gasUsed}`, life: 5000 })
  } catch (error: any) {
    toast.add({ severity: 'error', summary: 'Error transferring WTIST', detail: error.message.toString(), life: 5000 })
...
<template>
  <Card style="height: auto; max-width: 50vh" class="m-auto">
    ...
    <template #content>
      <div class="flex flex-col gap-4">
          ...
          <InputGroup>
            ...
            <FloatLabel variant="in">
              <InputText
                ...
                @keydown="isInvalidAddress = false"
              />
          ...
          <InputGroup>
            ...
            <FloatLabel variant="in">
              <InputText
                ...
                @keydown="isInvalidAmount = false ; isInsufficientBalance = false"
              />
        ...
        <Message severity="info" size="small" v-if="transferInfo.amount !== undefined && transferInfo.amount > 0">
          <p>
            Your transfer is equal to {{ transferInfo.amount }} barrels of oil and to ${{ wtiToUsd(transferInfo.amount) }}.
          </p>
        </Message>

        <Message severity="error" size="small" v-if="isInsufficientBalance">
          <p>
            Insufficient balance. Your balance is {{ props.balance }} WTIST.
          </p>
        </Message>
      </div>
    </template>
  ...
  >
    <p>
      Are you sure you want to execute the following transfer?
            ...
            <td>
              <div class="flex flex-row gap-2 items-center">
                <Button
                  ...
                  variant="outlined"
                />
                <span v-if="estimatedGas">{{ estimatedGas }} Gas</span>
              </div>
            </td>
          </tr>
        </tbody>
    ...
    <template #footer>
      <Button label="Confirm" class="p-button-success" @click="approveTransfer" :loading="loadingTransfer" :disabled="loadingTransfer" />
      <Button label="Cancel" class="p-button-danger" @click="showApproveDialog = false; loadingTransfer = false" :disabled="loadingTransfer" />
    </template>
  </Dialog>
</template>
