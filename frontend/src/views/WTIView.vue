...
export default {
  ...
  methods: {
    ...
    async mintWTI() {
      ...
      console.log("Collateral Amount:", this.collateralAmount);
    ...
    // Send WTI tokens to another address
    async sendWTI() {
      if (!this.recipientAddress || !this.sendAmount || this.sendAmount <= 0) {
        alert("Enter a valid recipient address and send amount.");
        return;
      }
      ...
      try {
        ...
        const tx = await this.contract.transfer(
            ...
            ethers.utils.parseUnits(this.sendAmount, 18)
        );
        await tx.wait();
        alert("WTI tokens sent successfully.");
        await this.fetchWTIData();
      } catch (error) {
        console.error("Error sending WTI tokens", error);
      }
    },
  },
};
